This supplement gives the generator, closed second-order hierarchy, exhaustive pairwise proof, restricted-identity ledger, and mandatory limits for the two-dimensional inertial active Brownian particle with deterministic Poisson resetting. It is written to be read independently. Repository links are included only as traceability pointers to the accepted symbolic records and implementations.

\section{S1. Model, reset semantics, and generator}\label{s1.-model-reset-semantics-and-generator}
...
Reset times are those of an independent Poisson process of rate \(\rho\). Each reset is instantaneous. With \(0\) denoting the appropriate scalar or two-component zero, the seven nonempty deterministic maps are
...
Traceability: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/docs/scientific-contract/CONTRACT.md}{scientific contract} and \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/src/phasemap/theory/generator.py}{symbolic generator implementation}.

\section{S2. Temporal scope of the second-order record}\label{s2.-temporal-scope-of-the-second-order-record}
...
\section{S3. Ordered redundant 28-coordinate frame and exact linear moment hierarchy}\label{s3.-ordered-redundant-28-coordinate-frame-and-exact-linear-moment-hierarchy}
...
or, in homogeneous coordinate form, \(\texttt{one}-U_{xx}-U_{yy}=0\). The symmetric matrices \(R,S,U\) retain three stored components, while the nonsymmetric matrices \(C,Q,W\) retain all four. Together with the constant and six first moments, the implementation inventory is \(1+6+9+12=28\) entries. The exact trace constraint makes this inventory a redundant moment frame. Retaining it keeps the accepted generator, formula, and serialization interfaces stable.
...
Protocols without orientation reset (\(\eta=0\)) preserve the trace constraint, while protocols with orientation reset (\(\eta=1\)) restore any trace deviation exponentially at rate \(\rho\). Thus every one of the seven generators leaves the physical invariant manifold unchanged. The generator's explicit physical-state validation entry point rejects states outside this manifold rather than assigning them a physical interpretation.
...
\subsection{S3.1 Long-time solution for position-reset protocols}\label{s3.1-long-time-solution-for-position-reset-protocols}
...
Traceability: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/T-020-position-protocols.md}{accepted position-protocol solution}.

\subsection{S3.2 Long-time solution for non-position protocols}\label{s3.2-long-time-solution-for-non-position-protocols}
...
Thus all three non-position protocols are diffusive in centered covariance. For \(V\), \(\bar v=0\), so raw MSD is also diffusive. For the two orientation-reset protocols,
...
Traceability: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/T-021-nonposition-protocols.md}{accepted non-position solution}.

\section{S4. Exact 21-pair witness table}\label{s4.-exact-21-pair-witness-table}
...
and that parameters and the common initial state are matched. The following three rows are disjoint and give the exact partition \(12 + 6 + 3 = 21=\binom72\) unordered pairs.
...
Exact pairs
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Separating member of \(\mathcal R_2\)
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
...
Traceability: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/T-030-memory-channel-proposition.md}{accepted exhaustive proposition}.

\section{S5. Restricted-identity ledger}\label{s5.-restricted-identity-ledger}
...
\subsection{S5.1 Internal-sector identities}\label{s5.1-internal-sector-identities}
...
\subsection{S5.2 Contracted-observable degeneracies}\label{s5.2-contracted-observable-degeneracies}
...
for all finite physical parameters. At \(\rho>0\), orientation moments separate the records; in the active interior, \(V\Theta\) also drifts and \(D_{\mathrm{eff},V}>D_{\mathrm{eff},V\Theta}\).

\subsection{S5.3 Resonant identity}\label{s5.3-resonant-identity}
...
\subsection{S5.4 Conditional identities}\label{s5.4-conditional-identities}
...
There is also an accepted analytic-only curve on which \(V\) and \(\Theta\) have the same centered diffusion tensor:
...
where \(M_\star\) is the unique positive root of \(44-9M-80M^2-12M^3=0\) (\(M_\star=0.658170892\ldots\)). The denominator is positive on the stated domain. This is not a full-record identity: \(\Theta\) has nonzero mean drift and ballistic raw MSD, whereas \(V\) has zero drift and diffusive raw MSD. The curve is an exact analytic identity and was not part of the sampled numerical validation grid.

\subsection{S5.5 Limiting identities}\label{s5.5-limiting-identities}

At fixed \(\rho>0\) and \(\mathrm{Pe}\ge0\), first derive the exact finite-\(M\) stationary position moments (for position-reset maps) or long-time drift/diffusion coefficients (for non-position maps), and then take the position-output limit \(M\to0^+\). In this order,
...
Traceability: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/V-020-equivalence-map.md}{accepted classification and identity ledger} and \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/T-030-memory-channel-proposition.md}{accepted proposition}.

\section{S6. Mandatory baselines and limits}\label{s6.-mandatory-baselines-and-limits}

\subsection{S6.1 Complete-reset baseline}\label{s6.1-complete-reset-baseline}
...
The complete component solution from S3.1 reproduces the accepted complete-reset inertial baseline.

\subsection{S6.2 Zero-rate and rare-reset limit}\label{s6.2-zero-rate-and-rare-reset-limit}
...
\subsection{S6.3 Passive underdamped limit}\label{s6.3-passive-underdamped-limit}
...
\subsection{S6.4 Singular overdamped position-process limit}\label{s6.4-singular-overdamped-position-process-limit}
...
For the localized protocols, the accepted procedure is: solve the exact finite-\(M\) stationary position moments at fixed \((\rho,\mathrm{Pe})\), then take \(M\to0^+\). This gives
...
\subsection{S6.5 Frequent-reset asymptotics}\label{s6.5-frequent-reset-asymptotics}
...
\section{S7. Claim boundary}\label{s7.-claim-boundary}

The exact result is full-record second-order distinguishability of all seven nonempty reset maps for \(M>0\), \(\rho>0\), and \(\mathrm{Pe}\ge0\), together with a scope-labeled ledger of information loss under internal-sector restriction, scalar contraction, special parameter conditions, resonance, and singular limits. Trace equality is not matrix equality; raw-MSD equality is not centered-covariance equality; and none of the restricted identities is full protocol equivalence.

The supplement makes no practical-identifiability claim and introduces no higher-order or distributional claim.

\section{S8. Source crosswalk}\label{s8.-source-crosswalk}

\begin{itemize}
\tightlist
\item
  Model, reset maps, required observables, identity labels, and mandatory limits: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/docs/scientific-contract/CONTRACT.md}{scientific contract}.
\item
  Exact generator, ordered redundant frame, and fail-closed closure construction: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/src/phasemap/theory/generator.py}{generator.py}.
\item
  Position-reset stationary solution and limits: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/T-020-position-protocols.md}{T-020}.
\item
  Non-position centered transport law and limits: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/T-021-nonposition-protocols.md}{T-021}.
\item
  Full-record proposition and exhaustive separating proof: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/T-030-memory-channel-proposition.md}{T-030}.
\item
  Exact classification, restricted identities, resonance, and tuned diffusion curve: \href{https://github.com/blakepi/phasemap-reproducibility/blob/v3.1.1/artifacts/derived/V-020-equivalence-map.md}{V-020}.
\end{itemize}
